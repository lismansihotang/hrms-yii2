            ...
            if ($file = \yii\web\UploadedFile::getInstance($model, 'fileXls') !== null) {
                ...
                if (count($data) > 0) {
                    foreach (array_values($data) as $value) {
						#var_dump($row,array_key_exists('AC-No.', $row));exit;
						foreach($value as $key=>$row){
                            ...
                            $arrDataAbsensi[(int)$row['AC-No.']][] = ['finger_id' => (int)$row['AC-No.'], 'tgl' => $date, 'in' => $timeIn, 'out' => $timeOut];
                            #var_dump($arrDataAbsensi);exit;
                        }
						}
